...
>(({ className, inset, children, ...props }, ref) => {
  ...
  return (
    <DropdownMenuPrimitive.SubTrigger
      ref={ref}
      className={cn(
        "flex cursor-pointer select-none items-center rounded-sm px-2 py-1.5  outline-none focus:bg-accent data-[state=open]:bg-accent",
        inset && "pl-8",
        className
      )}
      {...props}
    >
      {children}
      <ChevronIcon className={cn("h-4 w-4 ms-auto")} />
    </DropdownMenuPrimitive.SubTrigger>
  );
});
